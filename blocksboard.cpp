...
Qt::Key BlocksBoard::holdButton = Qt::Key_H;
...
void BlocksBoard::setHoldPieceLabel(QLabel *label)
{
...
{
    return QSize (BoardWidth * 5 + QFrame::frameWidth() * 2,
                  BoardHeight * 5 + QFrame::frameWidth() *2);
...
void BlocksBoard::showNextPiece()
{
    ...
    //painter.setCompositionMode(QPainter::compo);
   // painter.fillRect(pixMap.rect(),nextPieceLabel->palette().background());
    painter.fillRect(pixMap.rect(),nextPieceLabel->palette().currentColorGroup());
...
Qt::Key BlocksBoard::getHoldButton()
{
    return holdButton;
}

void BlocksBoard::setHoldButton(const Qt::Key &key)
{
    holdButton = key;
}
...
void BlocksBoard::changeCurrentPiece()
{
    ...
    if(held == false){
        ...
        //update hold piece label with the piece being held
        ...
        //replace held piece with next piece
        //update hold piece label with the piece being held
        int dx = holdPiece.maxX() - holdPiece.minX() + 1;
        int dy = holdPiece.maxY() - holdPiece.minY() + 1;
